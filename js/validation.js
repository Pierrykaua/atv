...
function validaCPF(event){
    ...
    inputCPF.value = !formatedCPF[2] ? formatedCPF[1] : formatedCPF[1] + '.' + formatedCPF[2] + '.' + formatedCPF[3] + (formatedCPF[4] ? '-' + formatedCPF[4] : '');
}


function validaRenda(event){
    const inputRenda = event.target;
    let formatedRenda = inputRenda.value.replace(/\D/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,3})/);
